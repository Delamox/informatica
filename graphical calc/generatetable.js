...
function draw() {
  ...
  for (let x = 0; x < Xaxis + 1; x += 1) {
    ...
    for (let m = 0; m < (Yaxis - dy + 1) - prevY - 1; m++) {
      try { table.rows[m + prevY + 0].cells[x - 1].style.background = 'dimgray' } catch { ; }
    }
    for (let m = 0; m < -1 * ((Yaxis - dy + 1) - prevY) - 1; m++) {
      try { table.rows[prevY - m - 2].cells[x - 2].style.background = 'dimgray' } catch { ; }
    }
    prevY = Yaxis - dy + 1
  }
}
